defmodule DataDiode.S1.TCPHandlerExtraTest do
  ...
  test "handle_info tcp_error logs and stops", %{state: state} do
    assert capture_log(fn ->
      {:stop, :shutdown, _new_state} = TCPHandler.handle_info({:tcp_error, state.socket, :einval}, state)
    end) =~ "S1: TCP error"
  end

  test "handle_info tcp_closed logs and stops", %{state: state} do
    assert capture_log(fn ->
      {:stop, :normal, _new_state} = TCPHandler.handle_info({:tcp_closed, state.socket}, state)
